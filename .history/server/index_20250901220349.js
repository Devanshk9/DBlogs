...
app.get('/api/blogs',async (req,res)=>{
    ...
    const result = await pool.query(`
      SELECT b.blog_id, b.title, b.content, b.user_id, u.username, b.created_at
      FROM blogs b
...
app.get("/api/blogs/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      `SELECT b.blog_id, b.title, b.content, u.user_id, u.username, b.created_at
       FROM blogs b
       JOIN users u ON b.user_id = u.user_id
       WHERE b.blog_id = $1`,
      [id]
...
app.get('/api/writers/:id',async(req,res)=>{
  try {
   ...
   const blogsResult = await pool.query(
    ...
    res.json({
      username: writerResult.rows[0].username,
